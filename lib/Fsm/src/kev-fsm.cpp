...
State::State(char const* name, void (*on_enter)(), void (*on_state)(), void (*on_exit)())
: on_enter(on_enter),
  on_state(on_state),
  on_exit(on_exit),
  name(name)
...
Fsm::Fsm(State* initial_state, TransitionCallback on_transition)
: m_current_state(initial_state),
  m_transitions(NULL),
  m_num_transitions(0),
  m_num_timed_transitions(0),
  m_initialized(false),
  m_on_transition(on_transition)
{
}
...
void Fsm::add_transition(State* state_from, State* state_to, Event event,
                         void (*on_transition)())
{
                               ...
                               unsigned long interval, void (*on_transition)())
{
  ...
  Transition transition = Fsm::create_transition(state_from, state_to, Event{-1, "TIMED"},
                                                 on_transition);
...
void Fsm::make_transition(Transition* transition)
{
  ...
  if (m_on_transition != nullptr)
    m_on_transition(*transition->state_from, *transition->state_to, transition->event);
...
bool operator==(Event a, Event b) { return a.id == b.id; }
bool operator!=(Event a, Event b) { return !(a == b); }
